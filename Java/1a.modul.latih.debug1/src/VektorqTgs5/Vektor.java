...
//                  x[i]=a;
    ...
    /*
    Bug : syntax, hasil, input, dll
    penanganannya : yang salah ditutup/di-comment
                    buat yang baru (sdh benar)
    
    di taruh di atasnya function
    berikan penjelasan secukupnya
    
    ngecek hasil dari suatu variabel nilaiX--> 
        tampilkan nilai variabel tsb baris per baris
        Sout("nilai x : "+nilaiX)
     ...
     */
     ...
     public Vektor substract2Vektor(Vektor v1,Vektor v2)
     {
      ...
      for(int i=0;i<v1.ukuran;i++)
          {
//           v3.x[i]=v1.x[i]+v2.x[i];
//           Seharusnya dikurangi, bukan ditambah
           v3.x[i]=v1.x[i]+v2.x[i];
          }
       return v3;
     }
...
//        Vektor v4=kuadratVektor(v1) ;
//        Salah dalam penulisan variabelnya
